...
Only the latest published version of Monarch CLI receives fixes. Please reproduce
against the current release before reporting:

```bash
npm install -g monarchai@latest
monarch --version
```
...
- the version (`monarch --version`), your OS and your terminal
- steps to reproduce, or a proof of concept
- anything you think we would get wrong on a first read

You will get an acknowledgement. Monarch CLI is a small project with no security
...
Monarch CLI handles provider API keys and reads your source. When attaching logs,
...
- Anything that lets a repository you merely *open* run code, read credentials,
  ...
  hooks and `MONARCH.md` is the area we care most about
- API keys leaking out of `~/.monarch/`, into a project directory, into a log, or
...
- The model doing something unhelpful, wrong, or unsafe when you approved it —
  Monarch asks before it acts, and an approved action is your decision
- Vulnerabilities in your chosen model provider's service or in third-party
  dependencies, unless Monarch's use of them is what creates the problem
